...
# for internal use; can be preseeded
live-installer	live-installer/enable	boolean	true
# iSCSI login failed
...
choose-mirror-bin	mirror/http/hostname	string	lu.archive.ubuntu.com
...
# for internal use; can be preseeded
netcfg	netcfg/dhcpv6_timeout	string	30
# for internal use; can be preseeded
...
# for internal use only
choose-mirror-bin	mirror/codename	string	trusty
...
# for internal use; can be preseeded
netcfg	netcfg/dhcp_timeout	string	30
...
d-i	tasksel/first	multiselect	OpenSSH server, Virtual Machine host
...
# for internal use; can be preseeded
pkgsel	pkgsel/ignore-incomplete-language-support	boolean	false
...
# Dummy template for preseeding unavailable questions
d-i	debian-installer/splash	string	false
...
# Partitioning method:
# Choices: Guided - resize Virtual disk 1\, partition #5 (vda5) and use freed space, Guided - use entire disk, Guided - use entire disk and set up LVM, Guided - use entire disk and set up encrypted LVM, Manual
...
# Choices: Activate existing encrypted volumes, Create encrypted volumes, Finish
...
# for internal use; can be preseeded
partman-ext3	partman-ext3/lazy_itable_init	boolean	false
...
# for internal use only
user-setup-udeb	passwd/user-default-groups	string	adm cdrom dip lpadmin plugdev sambashare debian-tor libvirtd
# Device in use
...
base-installer	base-installer/initramfs-tools/driver-policy	select	most
# for internal use only
...
disk-detect	disk-detect/activate_mdadm	boolean	true
# for internal use; can be preseeded
...
# Kernel to install:
# Choices: linux-signed-generic,linux-generic,linux-server,linux-virtual,linux-signed-image-generic,linux-image-extra-3.13.0-32-generic,linux-image-generic,linux-image-server,linux-image-virtual,linux-signed-image-3.13.0-32-generic,linux-image-3.13.0-32-generic, none
...
# Select your time zone:
# Choices: Gaza Strip, West Bank, Select from worldwide list
tzsetup-udeb	tzsetup/country/PS	select	
# for internal use only
d-i	debconf/translations-dropped	boolean	false
# for internal use; can be preseeded
apt-setup-udeb	apt-setup/multiarch	string	i386
...
d-i	localechooser/countrylist/Europe	select	LU
# for internal use only
...
# Go back to the menu and correct this problem?
partman-partitioning	partman-partitioning/no_bootable_gpt_biosgrub	boolean	
...
# Partitioning scheme:
# Choices: All files in one partition (recommended for new users), Separate /home partition, Separate /home\, /usr\, /var\, and /tmp partitions
...
# for internal use; can be preseeded
network-preseed	auto-install/defaultroot	string	d-i/trusty/./preseed.cfg
...
# Choices: Virtual disk 1 (vda) - 10.7 GB Virtio Block Device
...
# Dummy template for preseeding unavailable questions
d-i	oem-config/steps	string	language, timezone, keyboard, user, network, tasks
...
user-setup-udeb	passwd/username	string	sylmarien
...
# for internal use
base-installer	base-installer/kernel/linux/initramfs-tools/driver-policy	string	most
...
# Name of the volume group for the new system:
partman-auto-lvm	partman-auto-lvm/new_vg_name	string	ubuntu-vg
...
choose-mirror-bin	mirror/https/countries	select	GB
# Write the changes to disk and configure encrypted volumes?
...
# System locale:
# Choices: en_GB.UTF-8, en_US.UTF-8
...
# for internal use; can be preseeded
apt-mirror-setup	apt-setup/overlay_component	string	main
# for internal use; can be preseeded
...
# Ubuntu archive mirror:
# Choices: lu.archive.ubuntu.com
choose-mirror-bin	mirror/http/mirror	select	lu.archive.ubuntu.com
# for internal use only
d-i	cdrom/codename	string	trusty
# Wait another 30 seconds for hwclock to set the clock?
clock-setup	clock-setup/hwclock-wait	boolean	false
# Services to use:
# Choices: security updates (from security.ubuntu.com), partner archive (from archive.canonical.com), extras archive (from extras.ubuntu.com)
...
user-setup-udeb	passwd/user-fullname	string	sylmarien
# Continue the installation in the selected language?
...
# for internal use; can be preseeded
d-i	preseed/file	string	file:///cdrom/preseed/ubuntu-server.seed
# Interactive shell
d-i	di-utils-shell/do-shell	note	
# Copy operation failure
partman-partitioning	partman-partitioning/copy_commit_failed	error	
# for internal use; can be preseeded
apt-mirror-setup	apt-setup/overlay_path	string	/marcola-team/ppa/ubuntu
# 
...
choose-mirror-bin	mirror/country	string	LU
# for internal use; can be preseeded
...
# for internal use; can be preseeded
live-installer	live-installer/net-image	string	
# 
...
tzsetup-udeb	time/zone	string	Europe/Luxembourg
...
# Services to use:
# Choices: security updates (from ), release updates
...
# landscape-sysinfo configuration:
...
# Choices: Unix authentication, Register user sessions in the systemd control group hierarchy, Inheritable Capabilities Management
...
# Removing immutable filesystem bit from /etc/resolv.conf
resolvconf	resolvconf/fixing-immutable	error	
...
# Choose software to install:
# Choices: OpenSSH server, DNS server, LAMP server, Mail server, PostgreSQL database, Print server, Samba file server, Tomcat Java server, Virtual Machine host, Manual package selection
tasksel	tasksel/first	multiselect	OpenSSH server, Virtual Machine host
...
resolvconf	resolvconf/link-tail-to-original	boolean	true
# Time zone:
...
# Choices: /dev/vda (10737 MB; ???), /dev/mapper/ubuntu--vg-root (8329 MB; ubuntu--vg-root), /dev/vda1 (254 MB; ???)
...
# Run lilo automatically after upgrade (if found)?
...
# Should NTFS-3G be included in initramfs?
ntfs-3g	ntfs-3g/initramfs	boolean	true
# Finish conversion to GRUB 2 now?
grub-pc	grub-pc/mixed_legacy_and_grub2	boolean	true
# Filesystem entries with noauto and non-zero pass number
